...
function testcase()
{
	...
	../clearpixel ${arguments} -i $img -o $outdir/img_${iterator}.png &>>$outdir/log_${iterator}.txt && _pass || _fail
	let iterator+=1
}
...
testcase "Preview picture with downscaling and custom upscale value" "-d 70% -u 4 -p"
